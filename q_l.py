    ...
    def run_episode(self):
        ...
        while done == False:
            ...
            if sub_turn == 0:
                ...
                for act in actions.shape[1]:
                    if(switch):
                        if actions[act][1] > maximum:
                            next_node = actions[act][1]
                            maximum = next_node
                            mode = actions[act][2:]


                    else:
                        if actions[act][1] < minimum:
                            next_node = actions[act][1]
                            minimum = next_node
                            mode = actions[act][2:]
                if(switch):
        ...
        for i in range (5):
            self.length.append(self.SL.shortest_path(i))
        self.reward = reward
        self.long_path = self.SL.long_path
        self.learn()
